...
<body>
    ...
    <div class='container'>
        ...
        <h5 class='px-2' id='chartSw'>Select Charts</h5>
        ...
        <select name="emp" id="emp" >
            ...
            <option value="<?php echo $rowem['user_id']; ?>">
                <?php echo $rowem['emp_fname'].' '.$rowem['emp_fname']; ?>
            </option>
            <?php endwhile; ?>
        </select>
        ...
        $sqlnew = "SELECT *, course.cou_name AS cou_realname FROM course
                ...
                INNER JOIN faculty ON faculty.fac_id = student.fac_id
                INNER JOIN branch ON branch.fac_id = faculty.fac_id 
                WHERE 1 AND course.cou_id = registra.cou_id AND student.user_id = registra.user_id";
        ...
        if(!empty($gender)){
            if ($gender == 'all') {
                $sql1 .= " AND student.stu_gender = 'ชาย'AND student.stu_gender = 'หญิง'";
            } elseif($gender == 'men') {
        ...
        $data = array();
        if($gender == 'all'){
            $data[] = array('cou_name','GPA Student', 'Grade in Class', 'Year class', 'men','women');
            while ($rownew = mysqli_fetch_assoc($resultnew)) {
                $data[] = array(
                    $rownew['stu_fname'], // Replace 'month' with the actual name of the column for month data in your table
                    (int)$rownew['stu_gpa'], // Replace 'bolivia' with the actual name of the column for Bolivia data in your table
                    (int)$rownew['grade'],// Replace 'madagascar' with the actual name of the column for Madagascar data in your table
                    ...
                    (int)$rownew['stu_year'], // Replace 'rwanda' with the actual name of the column for Rwanda data in your table
                    (int)$rowCount_male,
            ...
            $data[] = array('cou_name','GPA Student', 'Grade in Class', 'Year class','Course credit', 'men');
            while ($rownew = mysqli_fetch_assoc($resultnew)) {
                $data[] = array(
                    $rownew['cou_name'], // Replace 'month' with the actual name of the column for month data in your table
                    (int)$rownew['stu_gpa'], // Replace 'bolivia' with the actual name of the column for Bolivia data in your table
                    (int)$rownew['grade'],// Replace 'madagascar' with the actual name of the column for Madagascar data in your table
                    //(int)$row['reg_score'],'score'  Replace 'papua_new_guinea' with the actual name of the column for Papua New Guinea data in your table
                    (int)$rownew['stu_year'],
                    (int)$rownew['cou_credit'], // Replace 'rwanda' with the actual name of the column for Rwanda data in your table
            ...
            while ($rownew = mysqli_fetch_assoc($resultnew)) {
                $data[] = array(
                    ...
                    //(int)$row['reg_score'],'score'  Replace 'papua_new_guinea' with the actual name of the column for Papua New Guinea data in your table
                    (int)$rownew['stu_year'],
                    (int)$rownew['cou_credit'] // Replace 'average' with the actual name of the column for Average data in your table
                );
            }
        }elseif($emp == 0 ){
            $data[] = array('cou_name','GPA Student', 'Grade in Class', 'Year class','Course credit');
            ...
            while ($rownew = mysqli_fetch_assoc($resultnew)) {
                $data[] = array(
                    ...
                    (int)$rownew['stu_gpa'], // Replace 'bolivia' with the actual name of the column for Bolivia data in your table
                    (int)$rownew['grade'],// Replace 'madagascar' with the actual name of the column for Madagascar data in your table
                    (int)$rownew['reg_score']/20,
                    (int)$rownew['stu_year'], 
                    (int)$rownew['cou_credit']
                );
            }
        }
